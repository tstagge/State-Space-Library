%% output = ssStep(SS, t_Vec, makePlot)
...
%   i) makePlot = [optionl argument, defaults to 1]
...
%% Numerical State Space Functions: ssStep()
% [v1.0] [2019.07.31]
%   -Calls ssResponse() using a unit step
% [v2.0] [2019.08.01]
...
% SS functions used directly:    ssResponse(), ssSettlingTime()
% SS functions used indirectly:  createSSmodel()

function output = ssStep(SS, t_Vec, makePlot)
    % INPUT BUFFERING
    if nargin==1 %if arguments are (SS)
        t_Vec = ssStep_defaultTime(SS);
        makePlot = 1;
    end
    ...
    output = ssResponse(SS, ones(length(t_Vec),SS.n_in), t_Vec, makePlot);
end
function t = ssStep_defaultTime(SS)
    dt = 0.001;
